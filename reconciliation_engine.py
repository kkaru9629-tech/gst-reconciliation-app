...
def parse_tally(df):
    ...
    # Tax Columns
    df["CGST"] = df[[c for c in df.columns if "input_cgst" in str(c).lower()]].applymap(parse_numeric).sum(axis=1)
    df["SGST"] = df[[c for c in df.columns if "input_sgst" in str(c).lower()]].applymap(parse_numeric).sum(axis=1)
    df["IGST"] = df[[c for c in df.columns if "input igst" in str(c).lower()]].applymap(parse_numeric).sum(axis=1)
...
# -------------------------------------------------------
# GSTR-2B PARSER (Exact GST Portal Format)
# -------------------------------------------------------
